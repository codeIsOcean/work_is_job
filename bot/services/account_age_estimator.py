...
class AccountAgeEstimator:
    ...
    def add_calibration_point(self, user_id: int, creation_timestamp: int) -> bool:
        """
        Добавляет новую калибровочную точку в маппинг и сохраняет в файл.

        Вызывается автоматически когда Pyrogram получает реальную дату создания аккаунта.
        Данные сохраняются в JSON файл и НЕ теряются при перезагрузке бота.

        Args:
            user_id: ID пользователя Telegram
            creation_timestamp: Unix timestamp реальной даты создания

        Returns:
            True если точка добавлена, False если уже существует или ошибка
        """
        try:
            # ─────────────────────────────────────────────────────────
            # Проверяем что точка ещё не существует (избегаем дубликатов)
            # ─────────────────────────────────────────────────────────
            existing_ids = {p[0] for p in self.mapping}
            if user_id in existing_ids:
                logger.debug(f"Точка user_id={user_id} уже существует в маппинге, пропускаем")
                return False
            ...
            # Сортируем по user_id для корректной интерполяции
            self.mapping = sorted(self.mapping, key=lambda x: x[0])

            # ─────────────────────────────────────────────────────────
            # Сохраняем в файл (данные НЕ теряются при перезагрузке)
            # ─────────────────────────────────────────────────────────
            self._save_mapping_to_file()

            # ─────────────────────────────────────────────────────────
            # Перестраиваем spline с новыми данными
            # ─────────────────────────────────────────────────────────
            self._build_spline()
            ...
            logger.info(
                f"📊 MAPPING_UPDATED: Добавлена точка user_id={user_id}, "
                f"date={creation_date.strftime('%Y-%m-%d')}, "
                f"всего точек: {len(self.mapping)}"
            )

            return True

        except Exception as e:
            logger.error(f"Ошибка добавления калибровочной точки: {e}")
            return False

    def _save_mapping_to_file(self) -> None:
        """
        Сохраняет текущий маппинг в JSON файл.

        Вызывается автоматически после добавления новой точки.
        """
        try:
            ...
            logger.debug(f"Маппинг сохранён в {self.mapping_file} ({len(self.mapping)} точек)")

        except Exception as e:
            logger.error(f"Ошибка сохранения маппинга в файл: {e}")
    ...
    # =========================================================================
    # ДИНАМИЧЕСКИЙ РАСЧЁТ ВОЗРАСТА (НОВЫЙ МЕТОД)
    # =========================================================================
    # Использует формулу: age_days = (max_seen_id - user_id) / DAILY_GROWTH
    # Не зависит от статического маппинга, автоматически адаптируется
    # =========================================================================

    # Константа: примерный прирост user_id в день (~2.5 миллиона)
    # Расчёт на основе реальных данных:
    # - 6.25B (2023) → 8.6B (дек 2025) = 2.35B за ~2.5 года = ~2.6M/день
    DAILY_GROWTH = 2_500_000

    # Ключ для хранения максимального ID в Redis
    REDIS_MAX_ID_KEY = "telegram:max_seen_user_id"

    # Fallback значение если Redis недоступен (декабрь 2025)
    _fallback_max_id = 8_600_000_000

    async def update_max_seen_id(self, redis_client, user_id: int) -> bool:
        """
        Обновляет максимальный известный user_id в Redis.

        Вызывать при каждом новом пользователе (join_request, сообщение).
        Это позволяет системе автоматически адаптироваться к росту Telegram.

        Args:
            redis_client: Клиент Redis (из bot.services.redis_conn)
            user_id: ID пользователя который только что появился

        Returns:
            True если ID обновлён, False если текущий ID меньше сохранённого
        """
        try:
            # Получаем текущий максимум из Redis
            current_max = await redis_client.get(self.REDIS_MAX_ID_KEY)

            # Преобразуем в int (или используем 0 если нет данных)
            current_max_int = int(current_max) if current_max else 0

            # Если новый ID больше — обновляем
            if user_id > current_max_int:
                # Сохраняем новый максимум в Redis
                await redis_client.set(self.REDIS_MAX_ID_KEY, str(user_id))

                # Обновляем локальный fallback
                self._fallback_max_id = user_id

                # Логируем обновление (только для значительных изменений)
                if user_id - current_max_int > 1_000_000:
                    logger.info(
                        f"📊 MAX_ID обновлён: {current_max_int:,} → {user_id:,} "
                        f"(+{user_id - current_max_int:,})"
                    )
            ...
            # При ошибке Redis — не падаем, просто логируем
            logger.warning(f"Ошибка обновления max_seen_id в Redis: {e}")
            return False

    async def get_max_seen_id(self, redis_client) -> int:
        """
        Получает максимальный известный user_id из Redis.

        Args:
            redis_client: Клиент Redis

        Returns:
            Максимальный user_id или fallback значение
        """
        try:
            ...
            if cached:
                # Обновляем локальный fallback
                self._fallback_max_id = int(cached)
                return self._fallback_max_id

        except Exception as e:
            # При ошибке Redis — используем fallback
            logger.warning(f"Ошибка получения max_seen_id из Redis: {e}")

        # Возвращаем fallback если Redis недоступен
        return self._fallback_max_id

    async def get_dynamic_age_days(self, redis_client, user_id: int) -> int:
        """
        Динамический расчёт возраста аккаунта в днях.

        Формула: age_days = (max_seen_id - user_id) / DAILY_GROWTH

        Преимущества:
        - Не зависит от статического маппинга
        - Автоматически адаптируется к росту Telegram
        - Чем больше пользователей видит бот — тем точнее расчёт
        ...
        Returns:
            Примерный возраст аккаунта в днях
        """
        # Получаем максимальный известный ID
        max_id = await self.get_max_seen_id(redis_client)

        # Вычисляем разницу между максимумом и проверяемым ID
        id_difference = max_id - user_id

        # Если разница отрицательная — это новейший аккаунт (0 дней)
        if id_difference <= 0:
            return 0

        # Вычисляем возраст: разница / прирост в день
    ...
    async def is_young_account_dynamic(
        ...
        """
        Проверяет, является ли аккаунт молодым (для мута).

        Использует динамический расчёт на основе max_seen_id.

        Args:
            redis_client: Клиент Redis
            user_id: ID пользователя
            threshold_days: Порог в днях (по умолчанию 30)

        Returns:
            Tuple[is_young: bool, age_days: int, reason: str]
        """
        # Получаем динамический возраст
        age_days = await self.get_dynamic_age_days(redis_client, user_id)

        # Проверяем порог
        is_young = age_days <= threshold_days

        # Формируем причину для логов
        if is_young:
            reason = f"Молодой аккаунт: ~{age_days} дней (порог: ≤{threshold_days})"
        else:
            reason = f"Старый аккаунт: ~{age_days} дней"

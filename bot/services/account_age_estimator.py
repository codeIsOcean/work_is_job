...
class AccountAgeEstimator:
    ...
    def create_default_mapping(self) -> None:
        ...
        default_mapping = [
            ...
            # 2023-2025: Реальные оценки (исправлено: убраны прогнозные даты из будущего)
            [5500000000, 1698796800],  # 1 ноября 2023 - ~5.5B
            [6000000000, 1709251200],  # 1 марта 2024 - ~6B
            [7000000000, 1722470400],  # 1 августа 2024 - ~7B
            [8000000000, 1730419200],  # 1 ноября 2024 - ~8B
            [9000000000, 1740787200],  # 1 марта 2025 - ~9B (консервативный прогноз)
        ]
    ...
    def get_account_age_days(self, user_id: int) -> int:
        ...
        # ЗАЩИТА: Если дата создания в будущем - значит ошибка в маппинге или экстраполяции
        # Считаем такой аккаунт СТАРЫМ (возраст 365 дней), чтобы НЕ мутить по ошибке

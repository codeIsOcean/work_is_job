...
class ScamPatternService:
    ...
    def extract_patterns_from_text(
        ...
        # ─────────────────────────────────────────────────────
        # Получаем нормализатор для обработки обфускации
        # ─────────────────────────────────────────────────────
        normalizer = get_normalizer()

        for line in lines:
            # ─────────────────────────────────────────────────
            # ШАГ 2: Нормализуем строку ПЕРЕД очисткой
            # Это решает проблему с обфусцированным текстом:
            # "Ш.И.Ш.Л.О" → "шишло", "κурьеρ" → "курьер"
            # "🅼🅴🅵" → "меф", "порохх" → "порох"
            # ─────────────────────────────────────────────────
            line_normalized = normalizer.normalize(line)

            # ─────────────────────────────────────────────────
            # ШАГ 3: Очищаем строку от спецсимволов
            ...
            # ─────────────────────────────────────────────────
            # ШАГ 4: Фильтруем слова по длине и стоп-словам
            ...
            # ─────────────────────────────────────────────────
            # ШАГ 5: Генерируем фразы разной длины

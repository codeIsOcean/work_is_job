...
class TextNormalizer:
    ...
    def normalize(self, text: str) -> str:
        ...
        # Удаляем все символы категории Mn (Mark, Nonspacing) и Mc (Mark, Spacing Combining)
        result = ''.join(
    ...
    def _collapse_spaced_letters(self, text: str) -> str:
        """
        Схлопывает пробелы между одиночными буквами.

        Обрабатывает обфускацию типа "К а з и н о" → "казино"
        ...
        """
        # Паттерн: находим последовательности одиночных букв разделённых пробелами
        # Пример: "к а з и н о" → группа из 6 букв
        # Минимум 3 буквы подряд чтобы не ломать нормальный текст
        spaced_pattern = re.compile(
            r'\b([а-яёa-z\d])\s+([а-яёa-z\d])\s+([а-яёa-z\d])(?:\s+([а-яёa-z\d]))*\b',
            re.IGNORECASE | re.UNICODE
        )

        def replace_spaced(match):
            # Собираем все буквы из матча

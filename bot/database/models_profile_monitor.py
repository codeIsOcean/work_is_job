...
class ProfileMonitorSettings(Base, ExportableMixin):
    ...
    # ─────────────────────────────────────────────────────────
    # Включить проверку аватарки пользователя через модуль Scam Media Filter
    # При входе проверяет фото профиля против базы скам-изображений (по хешу)
    # Если совпадение найдено — применяет действие из настроек Scam Media Filter
    # По умолчанию выключено — требует включённого модуля Scam Media
    check_profile_photo_filter = Column(Boolean, default=False, nullable=False)

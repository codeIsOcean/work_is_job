...
class ContentFilterSettings(Base, ExportableMixin):
    ...
    # ─────────────────────────────────────────────────────────
    # Накапливает скор паттернов через несколько сообщений одного юзера.
    # Позволяет ловить спаммеров которые разбивают спам на части.
    # Каждая часть по отдельности не превышает порог раздела,
    # но в сумме за временное окно — превышает cross_message_threshold.

    # Включить/выключить кросс-сообщение детекцию
    # По умолчанию выключен — админ должен явно включить
    cross_message_enabled = Column(Boolean, default=False, nullable=False)

    # Временное окно накопления скора в секундах
    # За это время суммируются баллы от всех сообщений юзера
    # Дефолт: 7200 секунд = 2 часа
    cross_message_window_seconds = Column(Integer, default=7200, nullable=False)

    # Порог срабатывания — накопленный скор для применения действия
    # Пример: 3 сообщения по 35 баллов = 105 → превышает порог 100
    # Дефолт: 100 баллов
    cross_message_threshold = Column(Integer, default=100, nullable=False)

    # Действие при превышении порога: mute/ban/kick
    # NULL = использовать default_action
    # Дефолт: mute
    cross_message_action = Column(String(20), default='mute', nullable=True)

    # ─────────────────────────────────────────────────────────
    # НАСТРОЙКИ УВЕДОМЛЕНИЙ КРОСС-СООБЩЕНИЙ
    # ─────────────────────────────────────────────────────────
    # Текст уведомления при муте
    # Плейсхолдеры: %user% = имя пользователя, %time% = длительность мута
    # NULL = использовать стандартный текст
    cross_message_mute_text = Column(String(500), nullable=True)

    # Текст уведомления при бане
    # Плейсхолдеры: %user% = имя пользователя
    # NULL = использовать стандартный текст
    cross_message_ban_text = Column(String(500), nullable=True)

    # Через сколько секунд автоматически удалять уведомление
    # NULL = не удалять автоматически
    cross_message_notification_delete_delay = Column(Integer, nullable=True)
...
# ============================================================
# МОДЕЛЬ: ПАТТЕРНЫ КРОСС-СООБЩЕНИЕ ДЕТЕКЦИИ
# ============================================================
# Отдельная таблица паттернов для кросс-сообщение детекции.
...
# - Кросс-сообщения накапливают score за несколько сообщений
# - Нужны ДРУГИЕ веса (ниже), чтобы не банить обычных юзеров
#
# Пример: "пиши в лс" в разделе = 20 баллов (сработает сразу)
#         "пиши в лс" в кросс = 5 баллов (накопится только при спаме)
# ============================================================
class CrossMessagePattern(Base, ExportableMixin):
    # Имя таблицы в базе данных PostgreSQL
    __tablename__ = 'cross_message_patterns'

    # ─── Настройки экспорта ───
    # Позволяет экспортировать паттерны при переносе настроек группы
    __export_key__ = 'cross_message_patterns'
    # Порядок импорта: после основных настроек (100+)
    __export_order__ = 450
    ...
    # Оригинальный паттерн как его ввёл админ
    # Может быть словом, фразой или regex
    # Максимум 500 символов (достаточно для любой фразы)
    pattern = Column(String(500), nullable=False)

    # Нормализованная версия паттерна для поиска
    # Приведена к нижнему регистру, без лишних пробелов
    # Используется TextNormalizer для l33tspeak → кириллица
    # index=True — для быстрого поиска по тексту
    normalized = Column(String(500), nullable=False, index=True)
    ...
    # ─────────────────────────────────────────────────────────
    # Как искать этот паттерн в тексте:
    # - "word" = как отдельное слово (с границами \b)
    #   Пример: "лс" найдёт "пиши в лс", но НЕ "лесной"
    # - "phrase" = как подстроку (в любом месте текста)
    #   Пример: "пассивный доход" найдёт фразу в любом контексте
    # - "regex" = как регулярное выражение (для продвинутых)
    #   Пример: "\d+\s*\$" найдёт "100$", "500 $", "1000$"
    # По умолчанию phrase — самый частый случай
    pattern_type = Column(String(20), default='phrase', nullable=False)

    # ─────────────────────────────────────────────────────────
    # ВЕС ПАТТЕРНА (влияет на накапливаемый скор)
    # ─────────────────────────────────────────────────────────
    # Сколько баллов добавить к накопленному скору
    # ВАЖНО: веса должны быть НИЖЕ чем в разделах!
    # Рекомендуемые значения:
    # - 3-5 = слабый сигнал (частые слова: "лс", "напиши")
    # - 8-12 = средний сигнал (характерные фразы)
    # - 15-25 = сильный сигнал (явный спам-признак)
    # По умолчанию 10 — средний вес
    ...
    # ─────────────────────────────────────────────────────────
    # Связь с таблицей Group для удобного доступа к данным группы
    group = relationship("Group", backref="cross_message_patterns")
    ...
    __table_args__ = (
        # Уникальное ограничение: один паттерн один раз в группе
        # Нельзя добавить одинаковый паттерн дважды в одну группу
        UniqueConstraint('chat_id', 'pattern', name='uq_cross_msg_pattern_chat_pattern'),

        # Составной индекс для поиска активных паттернов группы
        # Ускоряет: SELECT * FROM cross_message_patterns WHERE chat_id=X AND is_active=True
        Index('ix_cross_msg_patterns_chat_active', 'chat_id', 'is_active'),
    )


# ============================================================
# ПОРОГИ БАЛЛОВ ДЛЯ КРОСС-СООБЩЕНИЕ ДЕТЕКЦИИ
# ============================================================
# Аналог CustomSectionThreshold но для кросс-сообщение детекции.
# Привязан к группе (chat_id), а не к разделу.
#
# Позволяет задать разные действия для разных диапазонов скора:
...
#
# Логика работы:
# 1. CrossMessageService накапливает score
# 2. При превышении cross_message_threshold ищем порог
# 3. Ищем порог где min_score <= score <= max_score
# 4. Если найден → применяем action и mute_duration из порога
# 5. Если НЕ найден → используем cross_message_action из ContentFilterSettings
# ============================================================
class CrossMessageThreshold(Base, ExportableMixin):
    # Имя таблицы в базе данных PostgreSQL
    __tablename__ = 'cross_message_thresholds'

    # ─── Настройки экспорта ───
    # Позволяет экспортировать пороги при переносе настроек группы
    __export_key__ = 'cross_message_thresholds'
    # Порядок импорта: после паттернов кросс-сообщений (450)
    __export_order__ = 451
    ...
    # Максимальный порог баллов (включительно)
    # NULL = без верхнего ограничения (∞)
    # Пример: max_score=149 означает score <= 149
    max_score = Column(Integer, nullable=True)
    ...
    # По умолчанию mute — самое частое действие
    action = Column(String(20), default='mute', nullable=False)

    # Длительность мута в минутах (только для action='mute')
    # 30 = 30 минут, 60 = 1 час, 1440 = 24 часа
    ...
    # True = порог активен и используется
    # False = порог отключён (не удалён, но не применяется)
    # Позволяет временно отключить порог без удаления
    enabled = Column(Boolean, default=True, nullable=False)
    ...
    # Порядок проверки порогов (меньше = проверяется раньше)
    # Если несколько порогов подходят — выбирается с меньшим priority
    # Обычно не нужен если диапазоны не пересекаются
    priority = Column(Integer, default=0, nullable=False)
    ...
    # ─────────────────────────────────────────────────────────
    # Описание порога для UI (опционально)
    # Пример: "Лёгкое нарушение - мут 30 минут"
    description = Column(String(200), nullable=True)
    ...
    # ─────────────────────────────────────────────────────────
    # Связь с таблицей Group для удобного доступа к данным группы
    group = relationship("Group", backref="cross_message_thresholds")
    ...
    __table_args__ = (
        # Индекс для поиска активных порогов группы с сортировкой по приоритету
        Index('ix_cross_msg_thresholds_chat_enabled', 'chat_id', 'enabled'),
    )
...
async def group_message_handler(
    ...
    # Проверяем анонимного админа (пишет от имени группы)
    # Если sender_chat == chat, это анонимный админ группы
    is_anonymous_admin = (
        message.sender_chat is not None
        and message.sender_chat.id == chat_id
    )

    # Проверяем обычного админа через API
    ...
    if is_admin:
        admin_type = "анонимный" if is_anonymous_admin else "обычный"
        logger.info(f"[COORDINATOR] 👑 Пользователь {user_id} - {admin_type} админ, пропускаем CF/AS")
        return

...
# - patterns.py: паттерны скама (добавление, удаление, список)
# - import_patterns.py: умный импорт паттернов из текста
...
# Импортируем роутер умного импорта паттернов (восстановлено из оригинала)
from bot.handlers.content_filter.scam.import_patterns import import_patterns_router
...
# Подключаем роутер умного импорта (должен быть после patterns для приоритета)
scam_router.include_router(import_patterns_router)

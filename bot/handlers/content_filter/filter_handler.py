...
async def _send_journal_log(
    ...
    # ─────────────────────────────────────────────────────────
        ...
        # CAS и БД спаммеров
        extra_info = ""
        # Показываем был ли пользователь уже в БД спаммеров бота
        if result.in_spammer_db:
            extra_info += "\n⚠️ <b>БД бота:</b> уже был в базе спаммеров"
        # Показываем CAS статус если проверка была выполнена
        if result.cas_checked:
            if result.cas_banned:
                extra_info += "\n🛡️ <b>CAS:</b> найден в базе → добавлен в БД"
            else:
                extra_info += "\n🛡️ <b>CAS:</b> не найден в базе"
        # БД спаммеров (по высокому порогу, не CAS)
        if result.added_to_spammer_db:
            extra_info += "\n📋 <b>БД:</b> добавлен (высокий score)"

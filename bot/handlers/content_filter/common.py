# ============================================================
# COMMON - FSM STATES И ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ
# ============================================================
# Этот модуль содержит:
# - Все FSM StatesGroup для Content Filter настроек
# - Вспомогательные функции парсинга (parse_duration, parse_delay_seconds)
# - Общие константы (WORDS_PER_PAGE, PATTERNS_PER_PAGE и др.)
#
# Вынесено из settings_handler.py для соблюдения SRP (Правило 30)
# ============================================================

# Импортируем FSM классы
from aiogram.fsm.state import State, StatesGroup
# Импортируем re для парсинга
import re
...
# Количество паттернов на странице для секций
SECTION_PATTERNS_PER_PAGE = 5

# Названия базовых сигналов на русском (для scam/base_signals.py)
BASE_SIGNAL_NAMES = {
...
# ============================================================
# FSM СОСТОЯНИЯ ДЛЯ КАТЕГОРИЙ СЛОВ
# ============================================================
...
# ============================================================
# FSM СОСТОЯНИЯ ДЛЯ КАСТОМНЫХ СЕКЦИЙ
# ============================================================
...
class SearchSectionPatternStates(StatesGroup):
    """FSM состояния для поиска паттернов в разделе.

    Позволяет искать паттерны по тексту и удалять найденные.
    """
    # Ожидание ввода поискового запроса
    waiting_for_query = State()
    # Ожидание ввода номера паттерна для удаления
    waiting_for_delete = State()


# ============================================================
# FSM СОСТОЯНИЯ ДЛЯ КРОСС-СООБЩЕНИЙ
# ============================================================

class AddCrossMessagePatternStates(StatesGroup):
    """FSM состояния для добавления паттерна кросс-сообщений.

    Паттерны кросс-сообщений используются для детекции спама,
    разбитого на несколько сообщений. Имеют ОТДЕЛЬНЫЕ веса
    от разделов антискама (обычно ниже).
    """
    # Ожидание ввода текста паттерна
    waiting_for_pattern = State()
    # Ожидание ввода веса паттерна
    waiting_for_weight = State()


class CrossMessageNotificationStates(StatesGroup):
    """FSM состояния для ввода текстов уведомлений кросс-сообщений.

    Позволяет настроить тексты уведомлений при муте или бане,
    с поддержкой плейсхолдеров %user% и %time%.
    """
    # Ожидание ввода текста уведомления при муте
    waiting_for_mute_text = State()
    # Ожидание ввода текста уведомления при бане
...
class CrossMessageWindowInputStates(StatesGroup):
    """FSM состояния для кастомного ввода временного окна кросс-сообщений.

    Позволяет админу ввести произвольное значение временного окна
    вместо выбора из предустановленных значений.

    Форматы ввода:
    - Число (секунды): 3600
    - С суффиксом: 30min, 2h, 1d
    """
    # Ожидание ввода временного окна в секундах
    waiting_for_window = State()


class CrossMessageThresholdInputStates(StatesGroup):
    """FSM состояния для кастомного ввода порога срабатывания.

    Позволяет админу ввести произвольное значение порога срабатывания
    вместо выбора из предустановленных значений (50, 75, 100...).
    """
    # Ожидание ввода порога в баллах
    waiting_for_threshold = State()


class CrossMessageCustomScoreStates(StatesGroup):
    """FSM состояния для кастомного ввода min/max score при добавлении порога.

    Позволяет админу ввести произвольные значения диапазона баллов
    для порогов вместо выбора из предустановленных (50, 75, 100...).
    """
    # Ожидание ввода минимального скора
...
class CrossMessageNotificationDelayInputStates(StatesGroup):
    """FSM состояния для кастомного ввода задержки автоудаления уведомления.

    Позволяет админу ввести произвольное значение задержки
    вместо выбора из предустановленных (10, 30, 60, 120, 300 сек).
    """
    # Ожидание ввода задержки в секундах
    waiting_for_delay = State()
...
class CrossMessageThresholdMuteInputStates(StatesGroup):
    """FSM состояния для кастомного ввода времени мута при добавлении порога.

    Позволяет админу ввести произвольное время мута
    вместо выбора из предустановленных (30мин, 2ч, 1д).
    """

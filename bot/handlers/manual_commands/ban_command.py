...
# ═══════════════════════════════════════════════════════════════════════════
# Этот файл содержит обработчик команды /aban:
# - Бан пользователя (ответом на сообщение или по @username/id)
...
# Импортируем парсер команд (используем тот же что для mute)
...
# Создаём роутер для команды бана
ban_router = Router(name="ban_command")
...
# ═══════════════════════════════════════════════════════════════════════════
# ХЕЛПЕР: ПАРСИНГ КОМАНДЫ /aban
# ═══════════════════════════════════════════════════════════════════════════
def parse_ban_command(text: str, has_reply: bool = False) -> ParsedCommand:
    """
    Парсит текст команды /aban и извлекает target и reason.

    Форматы команды:
    1. /aban                 → reply
    2. /aban причина         → reply с причиной
    3. /aban @username       → бан @username
    4. /aban @username спам  → бан @username с причиной
    5. /aban 123456789       → бан по user_id
    6. /aban 123456789 спам  → бан по user_id с причиной
    ...
    # Поддерживаем /aban и /aban@botname
    text = re.sub(r'^/aban(@\w+)?\s*', '', text, flags=re.IGNORECASE).strip()
...
def build_journal_message(
    ...
    Args:
        ...
        target_user: Объект User (цель бана)
        target_id: ID цели
        admin_user: Объект User (админ)
        reason: Причина
        added_to_spammers: Добавлен ли в БД спаммеров
        banned_groups_count: Количество групп где забанен
    ...
    if banned_groups_count > 1:
        extra_lines += f"\n🌍 <b>Забанен в {banned_groups_count} группах</b>"
...
def build_journal_keyboard(
    ...
    buttons.append(
        InlineKeyboardButton(
            text="🔓 Разбан",
            callback_data=f"mc:unban:{target_id}:{chat_id}"
        )
    )
...
# ХЕНДЛЕР КОМАНДЫ /aban
# ═══════════════════════════════════════════════════════════════════════════
@ban_router.message(Command("aban"))
async def handle_aban_command(
    message: Message,
    bot: Bot,
    session: AsyncSession,
):
    """
    Обработчик команды /aban.

    Форматы:
    - /aban                  → бан reply
    - /aban причина          → бан reply с причиной
    - /aban @user спам       → бан @user с причиной
    - /aban 123456 скам      → бан user_id с причиной
    """
    ...
    parsed = parse_ban_command(message.text, has_reply=has_reply)

    # ─── Шаг 4: Определяем цель бана ───
    target_id = 0
    target_user = None

    if parsed.target_type == 'reply':
        # Бан по reply на сообщение
        if not message.reply_to_message:
            await message.reply(
                "❌ Ответьте на сообщение пользователя которого хотите забанить\n"
                "Или укажите @username или ID: <code>/aban @user причина</code>",
                parse_mode="HTML"
            )
            return
        ...
        # Бан по @username — пока не поддерживается
        await message.reply(
            "❌ Бан по @username пока не поддерживается.\n"
            "Ответьте на сообщение пользователя или используйте его ID:\n"
            "<code>/aban 123456789 причина</code>",
            parse_mode="HTML"
        )
        return
    ...
    if target_id == 0:
        await message.reply("❌ Не удалось определить пользователя для бана")
        return
    ...
    ban_result = await apply_ban(
        ...
        add_to_spammers=True,  # Всегда добавляем в БД спаммеров
        cross_group_ban=True,  # Всегда баним кросс-группово
    )

    # ─── Шаг 7: Проверяем результат ───
    if not ban_result.success:
        await message.reply(f"❌ {ban_result.error}")
        return

    # ─── Шаг 8: Удаляем сообщение нарушителя (если включено) ───
    if settings.ban_delete_message and message.reply_to_message:
        delete_delay = getattr(settings, 'ban_delete_delay', 0) or 0
        if delete_delay > 0:
            # Отложенное удаление
    ...
    if settings.ban_notify_group:
        ...
        custom_text = getattr(settings, 'ban_notify_text', None)
        if custom_text:
            ...
            # Стандартный текст
            notify_text = f"🚫 {user_link} забанен."

            # Добавляем причину если есть
            if parsed.reason:
                notify_text += f"\n📝 Причина: {html.escape(parsed.reason)}"

            # Добавляем информацию о кросс-групповом бане
            if ban_result.added_to_spammers:
                notify_text += "\n🗃️ Добавлен в БД спаммеров"
            if len(ban_result.banned_groups) > 1:
                notify_text += f"\n🌍 Забанен в {len(ban_result.banned_groups)} группах"

        notify_message = await message.answer(notify_text, parse_mode="HTML")

        # Отложенное удаление уведомления (если настроено)
        notify_delete_delay = getattr(settings, 'ban_notify_delete_delay', 0) or 0
    ...
    if journal and journal.journal_channel_id:
        try:
            ...
            journal_text = build_journal_message(
                ...
                reason=parsed.reason,
                added_to_spammers=ban_result.added_to_spammers,
                banned_groups_count=len(ban_result.banned_groups),
                chat_id=message.chat.id,
    ...
    logger.info(
        f"[MANUAL_CMD] /aban completed: target={target_id}, "
        f"banned_in={len(ban_result.banned_groups)} groups, "

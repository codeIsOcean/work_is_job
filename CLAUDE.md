...
## 🖥️ СЕРВЕР: Важная информация

**Хостинг:** VDSina (Россия)

**Проблема (январь 2026):** РКН периодически блокирует Google DNS (8.8.8.8) для некоторых IP.
Это приводит к тому что боты не могут подключиться к Telegram API.

**Решение:** Настроены множественные DNS серверы с автоматическим fallback:

```
# /etc/resolv.conf (система)
nameserver 1.1.1.1    # Cloudflare (основной)
nameserver 1.0.0.1    # Cloudflare (резерв)
nameserver 77.88.8.8  # Yandex (РКН не блокирует)
nameserver 8.8.8.8    # Google (если разблокируют)

# /etc/docker/daemon.json (для контейнеров)
{"dns": ["1.1.1.1", "1.0.0.1", "77.88.8.8", "8.8.8.8"]}
```

**Если боты перестали работать:**
1. Проверь `ping 1.1.1.1` и `ping api.telegram.org` с сервера
2. Если DNS не работает — проверь `/etc/resolv.conf`
3. После изменения DNS — перезапусти Docker: `systemctl restart docker`

---

## ⚠️ ВАЖНЫЙ КОНТЕКСТ: БОТ ПОД АКТИВНОЙ АТАКОЙ СКАММЕРОВ

**Дата:** Январь 2026

**Ситуация:**
- Бот эффективно блокирует скам (80%+ автоматически)
- Скаммеры начали угрожать владельцу в ЛС ("разъебём твои чаты")
- Скаммеры используют **продвинутые инструменты**:
  - Автопрохождение капчи (emoji, математика, картинки через AI vision)
  - Gemini API для генерации уникальных сообщений (обход дубликат-фильтров)
  - Множественные аккаунты с рандомизацией device/api_id
  - FloodWait защита (30-60 сек между действиями)

**Приоритет разработки:**
1. Усиление защиты от AI-генерируемого спама
2. Кросс-групповая детекция (один скаммер в нескольких группах)
3. Кросс-сообщение детекция (разбивка спама на части)
4. Интеграция Gemini для контр-атаки (извлечение паттернов, классификация)

**Скриншоты программы скаммеров:** `docs/content_filter/scam_prog.jpg`, `scammer_threat.jpg`, `scammer_threat1.jpg`

**Задачи:** `docs/NEXT_SESSION_TASKS.md` (3 задачи в очереди)
...
### 🔴 КРИТИЧЕСКИЙ БАГ (январь 2026) — автоодобрение при блокировке бота:
```
Контекст: Claude в одной из сессий добавил "fallback" логику в captcha_coordinator.py
Логика: если не удалось отправить капчу → автоматически одобрить заявку

Что произошло:
1. Пользователь вошёл в группу A, получил мут за молодое фото
2. Пользователь заблокировал бота (обиделся)
3. Пользователь подал заявку в группу B
4. Бот не смог отправить капчу (Forbidden: bot was blocked)
5. БАГ: Бот автоматически ОДОБРИЛ заявку в закрытую группу!
6. Telegram API не отдал фото (бот заблокирован) → 0 фото
7. Пользователь прошёл БЕЗ проверки, хотя 9 минут назад был замучен

Доказательство: "KindMovers joined the group via public request, approved by Aika_test"

Результат: скаммеры могут обходить капчу и проверку профиля,
просто заблокировав бота после первого входа!
```

### ⛔ ПРАВИЛО: ВСЕГДА ПРОВЕРЯТЬ EDGE CASES!

При написании любой логики с fallback/обработкой ошибок — спроси себя:
- Что будет если пользователь заблокировал бота?
- Что будет если Telegram API вернул пустой ответ?
- Что будет если операция не удалась — fallback безопасен?
- Может ли злоумышленник эксплуатировать этот fallback?

**НИКОГДА не делать автоодобрение как fallback при ошибке!**
Если что-то пошло не так → НЕ одобрять, оставить заявку висеть, уведомить в журнал.
...
## ⛔ ГЛУБОКИЙ АНАЛИЗ ПЕРЕД КРИТИЧЕСКИМИ ДЕЙСТВИЯМИ (MUST!!!)

**⚠️ КРИТИЧЕСКОЕ ПРАВИЛО — добавлено после инцидента с паттернами (январь 2026)**

### История инцидента:
```
Задача: удалить дубликаты паттернов в разделе "Наркотики"
Что предложил Claude: удалить 157 "дубликатов"
Реальность: это были РАЗНЫЕ паттерны для разных вариаций спама!
Последствия (если бы удалили): скаммеры прошли бы без проблем
```

### Что считается КРИТИЧЕСКИМ (требует глубокого анализа):

**БД и данные:**
- Удаление/изменение паттернов антиспама/контент-фильтра
- Изменение весов/порогов срабатывания
- Массовые UPDATE/DELETE в таблицах настроек
- Любые действия влияющие на детекцию спама

**Код и функционал:**
- Изменение логики хендлеров (капча, антиспам, муты)
- Добавление/удаление функций в критических модулях
- Рефакторинг рабочего кода
- Изменение сигнатур функций которые вызываются извне
- Удаление "неиспользуемого" кода без проверки

**Новый код:**
- Добавление нового модуля/хендлера
- Интеграция с существующим функционалом
- Изменение middleware или координаторов

### ЗАПРЕЩЕНО:
- Предлагать массовые изменения без глубокого анализа
- Называть паттерны "дубликатами" только по похожему тексту
- Удалять/изменять данные в БД без понимания функционала
- Удалять код который "кажется неиспользуемым" без проверки
- Рефакторить "заодно" с основной задачей
- Менять логику без понимания всех зависимостей

### ОБЯЗАТЕЛЬНО перед критическими действиями:

**Для БД:**
1. Понять функционал каждой записи
2. Проверить пересечения (что ловит одно и то же)
3. Проанализировать влияние удаления
4. Показать детальный анализ с объяснением
5. Спросить подтверждение

**Для кода:**
1. Прочитать существующий код полностью
2. Найти все места где используется (grep, LSP)
3. Понять зачем код написан именно так
4. Проверить тесты — что они покрывают
5. Оценить влияние на смежные модули
6. Спросить подтверждение перед изменением

### Пример правильного анализа (БД):
```
❌ ПЛОХО: "157 дубликатов, удалить?"

✅ ХОРОШО:
"Нашёл паттерны с одинаковым текстом:
- ID 46 и 568: 'шишло' (phrase, 70 баллов) — реальный дубликат записи
- ID 46 'шишло' и ID 1055 'шишки' — РАЗНЫЕ паттерны!
  'шишло' ловит обфускацию, 'шишки' — прямое слово

Рекомендую удалить только ID 568 (точный дубликат).
Подтверждаешь?"
```

### Пример правильного анализа (код):
```
❌ ПЛОХО: "Функция X не используется, удалю"

✅ ХОРОШО:
"Функция X вызывается в:
- coordinator.py:234 — при обработке сообщений
- filter_handler.py:89 — при проверке паттернов
Также есть тест test_function_x.py

Удаление сломает оба модуля. Точно удалять?"
```

### Документация:
- Работа с паттернами в БД: `docs/pattern_strategy.md`
- Правила разработки: `docs/DEVELOPER_RULES.md`
...
- **Чек-лист проверки задач:** `docs/CHECKLIST.md` (ОБЯЗАТЕЛЬНО после каждой задачи!)
- **Стратегия паттернов:** `docs/pattern_strategy.md` (ВАЖНО для работы с БД!)
...
*Последнее обновление: 2026-01-17* (Добавлен CHECKLIST.md для проверки завершённых задач)